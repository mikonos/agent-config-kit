...
def build_ledger(
    ...
    portable_reference_aliases = (
        previous.get("portable_reference_aliases", {})
        if isinstance(previous, dict)
        else {}
    )
    if (
        not isinstance(portable_reference_aliases, dict)
        or any(
            not isinstance(private, str)
            or not isinstance(public, str)
            or not private
            or not public
            for private, public in portable_reference_aliases.items()
        )
    ):
        raise InventoryError("invalid portable reference aliases")
    ...
    for name, entry in sorted(selected.items()):
        ...
        public_name = (
            prior.get("public_name", name)
            if isinstance(prior, dict)
            else name
        )
        portable_aliases = (
            prior.get("portable_aliases", {})
            if isinstance(prior, dict)
            else {}
        )
        if (
            not isinstance(public_name, str)
            or not isinstance(portable_aliases, dict)
            or any(
                not isinstance(private, str)
                or not isinstance(public, str)
                for private, public in portable_aliases.items()
            )
        ):
            raise InventoryError(f"invalid previous portable identity: {name}")
        decision_status = decision.get("status")
        if decision_status not in (None, "blocked", "review"):
            raise InventoryError(f"invalid manual disposition status: {name}")
        group_name = admitted.get(public_name)
        ...
        if group_name is not None and not isinstance(group, dict):
            raise InventoryError(
                f"Skill references invalid provenance group: {public_name}"
            )
        status = (
            "bundled"
            ...
            **(
                {
                    ...
                    "portable_aliases": portable_aliases,
                }
                if public_name != name or portable_aliases
                else {}
            ),
            ...
            "disposition": {
                ...
                "category": (
                    pack_by_skill.get(public_name)
                    if group_name is not None
                    else (external_record.get("pack") if external_record else None)
                ),
                "default_install": public_name in default_skills,
    ...
    claimed_public_names = {
        record.get("public_name", name)
        for name, record in skills.items()
        if record["disposition"]["status"] == "bundled"
    }
    catalog_only = sorted(set(admitted) - claimed_public_names)
    ...
    return {
        ...
        **(
            {"portable_reference_aliases": portable_reference_aliases}
            if portable_reference_aliases
            else {}
        ),
        "skills": skills,
    }
