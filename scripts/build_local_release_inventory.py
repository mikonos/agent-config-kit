...
def build_inventory(
    ...
    for name, source_record in sorted(source_skills.items()):
        ...
        public_name = source_record.get("public_name", name)
        if (
            not isinstance(public_name, str)
            or not re.fullmatch(r"[A-Za-z0-9][A-Za-z0-9._-]*", public_name)
            or public_name in claimed_public_names
        ):
            raise InventoryError(f"invalid or duplicate public Skill name: {name}")
        claimed_public_names.add(public_name)
        if public_name in bundled:
            skills[public_name] = {
                "scope": "top_level",
                "delivery": "bundled",
                "pack": bundled[public_name],
            }
            continue
        if name in fetched:
            skills[name] = {
    ...
    catalog_only = sorted(set(bundled) - set(skills))
