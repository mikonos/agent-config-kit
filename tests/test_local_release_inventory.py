...
class LocalReleaseInventoryTests(unittest.TestCase):
    def test_builder_exports_only_public_alias_for_bundled_private_name(self) -> None:
        source = {
            "schema_version": 1,
            "skills": {
                "private-product-cloud": {
                    "public_name": "product-cloud-operations",
        ...
        manifest = {
            "schema_version": 1,
            "skill_packs": {
                "portable": ["product-cloud-operations"]
            },
        }
        ...
        self.assertIn("product-cloud-operations", result["skills"])
        self.assertNotIn("private-product-cloud", json.dumps(result))
    ...
    def test_current_public_inventory_is_complete(self) -> None:
        ...
        self.assertEqual(summary["raw_source_entry_count"], 445)
        self.assertEqual(summary["raw_source_unique_count"], 400)
        self.assertEqual(summary["raw_duplicate_names"], 43)
        self.assertEqual(summary["top_level_unique_count"], 365)
        self.assertEqual(summary["nested_only_unique_count"], 30)
        self.assertEqual(summary["platform_builtin_only_unique_count"], 5)
        self.assertEqual(summary["public_bundled_total"], 320)
        self.assertEqual(summary["redacted_private_projects"], 24)

    def test_builder_resolves_bundled_external_and_blocked_entries(self) -> None:
        source = {
            "schema_version": 1,
            "skills": {
                "example-private": {
