...
Also explain that `doctor` compares installed files with the selected Kit
release. Its `current` count does not prove that the release still matches the
maintainer's live source trees; that is a separate maintainer-only source
alignment gate required before publication.
