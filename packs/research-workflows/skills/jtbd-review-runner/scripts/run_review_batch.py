...
def parse_args() -> argparse.Namespace:
    parser = argparse.ArgumentParser(description="Run a batch of reviews through Cursor Agent.")
    parser.add_argument(
        "--review-ids",
        nargs="+",
        required=True,
        help="One or more review IDs, e.g. skylight15-0067 skylight15-1718",
    )
    ...
    parser.add_argument(
        "--payload-dir",
        default="",
        help="Optional directory to save per-review payloads",
    )
    ...
    parser.add_argument(
        ...
        type=int,
        default=0,
        help="Retry count for timeout/error cases",
    )
    parser.add_argument(
    ...
    parser.add_argument("--dry-run", action="store_true", help="Only write payloads / print summary")
    parser.add_argument(
        "--force-rerun",
        action="store_true",
        help="Re-run reviews even if output JSON already exists",
    )
...
def try_run_review(
    ...
    return build_result(
        ...
        output_path="",
        payload_path=payload_path,
        error=last_error or "Unknown runner failure",
    )
...
def main() -> None:
    ...
    summary_path = Path(args.summary_out) if args.summary_out else output_dir / "batch_summary.json"
    ...
    for review_id in args.review_ids:
        ...
        if output_path.exists() and not args.force_rerun:
            ...
            write_summary(summary_path, output_dir, args.review_ids, results)
