---
name: structure-note
description: 写文章/串联论证/整理项目文档，产出带阅读顺序和逻辑树的结构笔记。关键词：结构笔记、文章、论证、大纲。
---

> **项目协议**：执行前读取当前项目的 Agent Rule、Vault 目录说明和 YAML 约定；默认先给结构草案，获得明确授权后才写笔记和反向链接。

# 结构笔记

> 索引规范以当前项目文档或已安装 `index-note` Skill 为准。
