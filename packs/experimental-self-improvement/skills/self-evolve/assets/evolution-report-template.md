...
在选定进化方案后，需要在用户所选项目的 `.agent-evolve/` 下创建一份进化报告：
...
* **方案 A**：[描述与拟议变更；执行前仍需用户批准] — [起止观察时间]
* **方案 B**：[描述与拟议变更；执行前仍需用户批准] — [起止观察时间] (如果是A/B Test)
* **切换点**：[方案A切换到B的具体时间]
* **提前终止条件**：[什么负面情况下直接判负回滚]
* **评估截止时间**：[具体如 2026-03-01T12:00:00Z，到期触发评价流]

## 实验日志指针
* 获取到的数据对比指标将追加写入：`.agent-evolve/[cycle_id].jsonl`
```
