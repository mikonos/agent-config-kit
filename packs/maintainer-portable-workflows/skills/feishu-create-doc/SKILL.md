...
> **OpenClaw Gateway**：无 MCP 工具 `feishu_mcp_create_doc` 时，用内置 **`feishu_doc`**，`action: create`，字段对应 `title`、`folder_token`（租户身份下须为已向应用开通写权限的父文件夹；**勿传**空字符串）。正文后续用 `feishu_doc` 的 `write`/`append`。
