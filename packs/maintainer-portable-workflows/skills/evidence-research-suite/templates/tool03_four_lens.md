...
- **Job Statement 表**（若输入含 raw user voice）：<文件名 + JS id 范围；若尚未抽取，先抽取再执行本工具>
- **语言要求**：zh-CN

## 输出主体

### 零、Job Statement 证据底座（raw voice 输入时必填）

| Job Statement id | 触发的四镜 | 关键原话 / source | 可用性 |
|---|---|---|---|
| JS-001 | Patterns / Shortcuts / Feelings / Contradictions | <quote + source_id> | 可进入洞察 / 待追问 / 不可用 |
...
- [ ] **Job Statement 证据层**：输入含访谈/评论/反馈时，已列 Job Statement id；缺 context / verbatim / desired outcome 的条目未进入洞察，只进待追问 —— 证据：抽样 2 条 JS id + 原文位置
