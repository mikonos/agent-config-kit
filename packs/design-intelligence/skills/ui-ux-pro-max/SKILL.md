...
The search script lives inside this skill's own directory, not the project directory. Resolve `<installed-skill-dir>` to the directory containing this loaded `SKILL.md`, then invoke the script by that full path. Do not infer it from the current working directory or assume a runtime-specific Skill root:

```bash
python "<installed-skill-dir>/scripts/search.py" "<query>" --domain <domain>
```
...
```bash
python "<installed-skill-dir>/scripts/search.py" "<product_type> <industry> <keywords>" --design-system [-p "Project Name"]
```
...
**Example:**
```bash
python "<installed-skill-dir>/scripts/search.py" "beauty spa wellness service" --design-system -p "Serenity Spa"
```
...
```bash
python "<installed-skill-dir>/scripts/search.py" "<query>" --design-system --persist -p "Project Name" --output-dir "<project-root>"
```
...
```bash
python "<installed-skill-dir>/scripts/search.py" "<query>" --design-system --variance <1-10> --motion <1-10> --density <1-10>
```
...
**Example:**
```bash
python "<installed-skill-dir>/scripts/search.py" "internal analytics dashboard" --design-system --variance 8 --motion 7 --density 8 -p "Ops Console"
```

### Step 3: Supplement with Detailed Searches (as needed)

```bash
python "<installed-skill-dir>/scripts/search.py" "<keyword>" --domain <domain> [-n <max_results>]
```
...
```bash
python "<installed-skill-dir>/scripts/search.py" "<keyword>" --stack <stack>
```
...
python "<installed-skill-dir>/scripts/search.py" "AI search tool modern minimal" --design-system -p "AI Search"

# Step 3: supplement
python "<installed-skill-dir>/scripts/search.py" "search loading animation" --domain ux

# Step 4: stack guidelines
python "<installed-skill-dir>/scripts/search.py" "suspense streaming bundle" --stack nextjs
```
