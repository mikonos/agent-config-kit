...
3. **用途**：思维顾问？决策参考？角色扮演？
4. **新建 or 更新**：是否已有该人物的Skill？检查当前 Runtime 的项目 Skill 根目录：Codex 用 `.agents/skills/`，Cursor 用 `.cursor/skills/`，Claude Code 用 `.claude/skills/`
...
**来源A：本地已有Skill**
扫描当前 Runtime 项目 Skill 根目录中的 `*-perspective/`，读取每个SKILL.md的description，匹配用户需求。已有Skill可以即插即用，不需要重新蒸馏。如果扫描结果为空（用户还没有任何perspective skill），跳过此步，只从来源B推荐。
...
```
<project-skill-root>/[person-name]-perspective/
├── SKILL.md                          # 最终产物
...
#### 工具辅助（如可用）
- 书籍：只使用作者或出版方公开文本、用户提供的合法副本，或用户有权访问的图书馆/数据库；不得绕过付费墙或版权限制。获授权文件存入 `sources/books/`
- 视频字幕获取（已提供脚本，直接调用）：
...
Phase 1启动前，**主动扫描当前 Runtime 的项目 Skill 根目录**，检查是否有可用于信息获取的skill。如果有，在调研中优先调用，比WebSearch更稳定高效：
...
#### Step 4: 输出
将完成的SKILL.md写入 `<project-skill-root>/[person-name]-perspective/SKILL.md`。
