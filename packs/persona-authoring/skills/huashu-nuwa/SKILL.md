...
3. **用途**：思维顾问？决策参考？角色扮演？
4. **新建 or 更新**：是否已有该人物的Skill？（检查 `.cursor/skills/` 目录）
...
**来源A：本地已有Skill**
扫描 `.cursor/skills/*-perspective/` 目录，读取每个SKILL.md的description，匹配用户需求。已有Skill可以即插即用，不需要重新蒸馏。如果扫描结果为空（用户还没有任何perspective skill），跳过此步，只从来源B推荐。
...
```
.cursor/skills/[person-name]-perspective/
├── SKILL.md                          # 最终产物
├── scripts/                          # 工具脚本（字幕下载/清洗/质量检查）
└── references/
    ├── checkpoint-log.md             # 流程检查点日志（必存）
    ...
    └── sources/                      # 一手素材（用户提供 + 网络下载）
        ├── README.md                 # raw source边界说明（必存）
        ├── source-manifest.md        # 高权重来源追踪（建议）
        ├── books/
        ├── transcripts/
        └── articles/
```
...
**完成检查**（自动执行）：
- [ ] 目录已创建
- [ ] `references/checkpoint-log.md` 已创建，记录每个硬暂停点的状态
- [ ] `references/sources/README.md` 已创建，说明 raw source 是否完整
...
- **检查点必须可审计**：Phase 1.5 / 2.5 / 4 / 5 的展示、用户确认、继续条件必须写入 `references/checkpoint-log.md`。没有记录的确认等于没发生。
- **禁止事后伪造确认**：如果某个检查点已经越过但没有用户确认记录，只能写成“流程证据缺失 / 事后补救”，不能补写成“当时已确认”。
- **路径真源服从项目协议**：本项目默认写入 `.cursor/skills/[person-name]-perspective/`；如果其他项目的 `AGENTS.md` / `CLAUDE.md` 明确指定不同 skill 真源，以当地协议为准，并在 `checkpoint-log.md` 记录路径 override。
- **raw source边界必须说明**：如果 `sources/` 只有引用摘要、没有下载原始文章/字幕/书籍摘录，必须在 `sources/README.md` 标注“研究摘要自包含；raw source snapshot 不完整”。
...
Phase 1启动前，**主动扫描 `.cursor/skills/` 目录**，检查是否有可用于信息获取的skill。如果有，在调研中优先调用，比WebSearch更稳定高效：
...
**硬暂停规则**：
- 没有用户确认，不进入 Phase 2。
- 如果用户在 Phase 0 已明确授权“无需中途确认，连续执行”，也必须在 `references/checkpoint-log.md` 记录这句授权、时间和适用范围；否则仍按硬暂停处理。
- 记录格式至少包含：展示摘要、用户确认原话/授权、确认时间、继续条件。
...
**硬暂停规则**：
- 没有用户确认，不进入 Phase 3 构建。
- 不允许把 Phase 2 产物完整当作“用户已确认”的替代证据；产物证据和流程确认是两件事。
- 如果未确认却继续了，后续只能在 `checkpoint-log.md` 标为 `PARTIAL / 流程证据缺失`，不能事后追认为合规。
...
| 模板Section | 填充来源 |
|------------|---------|
| frontmatter description | 250–450 字符的路由合同：独特任务、隐式用户表达、相邻分流与反触发；不写来源数、模型数、调研日期或表达 DNA |
| frontmatter metadata | `type`、`research_date` 与 routing 校验开关；不得把这些字段放在顶层 |
...
#### Step 4: 输出
将完成的SKILL.md写入 `.cursor/skills/[person-name]-perspective/SKILL.md`。
...
**硬暂停规则**：
- Phase 4 通过只能说明“内容质量可交付”，不能自动等同“流程完成”。
- 必须把 sanity / edge / voice / 反触发结果写入验证文件，并在 `checkpoint-log.md` 记录用户确认后，才能宣告 skill 完成。
- 如果用户要求继续 Phase 5 精炼，也要记录为“确认进入 Phase 5”，而不是“确认最终完成”。
...
**硬暂停规则**：
- 应用改动后，必须展示：Agent A/B 主要建议、已应用改动、未应用改动及理由、对 `SKILL.md` 的影响。
- 用户确认后，在 `checkpoint-log.md` 记录最终交付状态。
- 如果缺少确认，最终判定必须写成“内容质量 PASS，女娲严格流程 PARTIAL PASS”，不能写成完全合规。
