...
description: |
  [人名]视角：[这个人物最独特的分析动作，以及它帮助用户得到的结果]。
  适用于[问题类型一]、[问题类型二]、[问题类型三]；即使用户没有点名[人名]，但提出
  「[真实用户表达一]」「[真实用户表达二]」「[真实用户表达三]」等同类问题，也应触发。
  分流：[相邻 skill 一]负责[边界一]；[相邻 skill 二]负责[边界二]；本 skill 只负责[本人物主任务]。
  不用于[反触发场景、安全边界或明显应由其他专业能力处理的问题]。
metadata:
  type: perspective
