...
description: Define product vision and strategy. Use when articulating long-term product direction.
---
...
5. **Check the need story** - If the vision is based on user research, require upstream Job Statements or equivalent user-scene evidence before turning it into product direction
...
Before writing the future world, anchor it in at least one present-world demand story: concrete `Context`, `Main Job`, `Desired Outcome`, and the user's own words. If those are missing, label the vision input as a founder hypothesis, not customer-backed direction.
...
- "Which Job Statement proves this user world is real rather than a feature wish?"
- "What concrete present-day workaround or compensation behavior shows users already pay this cost?"
...
- **No demand story** - Jumping from interview quotes or founder belief directly to vision without a Job Statement / equivalent need story

## Job Statement Gate

For customer-backed product vision, read `.cursor/skills/jtbd-result-audit/references/job_statement_quality_gate.md` and confirm:

- `Context` shows the present user world clearly.
- `Main Job` is a user task, not a product capability.
- `Desired Outcome` defines what a better future must change.
- `Compensation Behavior` or `Current Solution` proves users already work around the problem.
- `Verbatim` keeps the user's language available for vision phrasing.

If this evidence is absent, write the output as a hypothesis vision and name the discovery work needed before it can guide roadmap decisions.
