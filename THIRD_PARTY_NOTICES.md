...
The `maintainer-portable-workflows` pack contains user-authorized portable
derivatives selected from the maintainer's active Skill configuration and
released as part of this Kit under the MIT License. Where a workflow names a
method, author, public project, or platform, that attribution remains with its
respective source and does not imply endorsement. The public derivatives remove
private project names, personal paths, account state, and unsafe account-control
instructions. In particular, `claude-code-privacy-audit` is a clean safety
rewrite: it supports read-only inspection and documented privacy controls, not
ban evasion, identity unlinking, device-identifier resets, or credential
deletion.

Within that pack, `advisory-board`, `darwin-skill`, `interview-designer`, and
`writing-x-posts` retain their actual source-repository attribution in the
machine catalog. They are included under the maintainer's explicit
representation that the Kit has permission to redistribute these selected
portable derivatives under MIT; the Kit does not claim their upstream work was
originally authored by Agent Config Kit contributors.
