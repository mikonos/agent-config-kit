...
同名 Skill 必须优先采用维护者当前真源中选中的版本；如果公开包因脱敏或
跨 Runtime 兼容而不同，每个差异都必须登记为便携派生。未经解释的同名异版、
遗漏文件或待审条目会阻止下一次发布。

“全部”有一份可检查的逐项账本：
[`catalog/local_release_inventory.json`](catalog/local_release_inventory.json)。
最初 445 个入口对应的 400 个唯一候选全部有最终去向：303 个已进入公开包，
27 个改由安装器从官方来源获取，70 个因平台内置、仅为嵌套子组件、私有项目、
旧副本、安全或可移植性边界而明确不分发，没有待审条目。其中 24 个
私有项目候选只以匿名编号、数量和排除理由记账，不公开原名。365 个是顶层候选，
另外 30 个只在其他 Skill 内嵌套出现，5 个只来自平台 `.system`。
公开 Catalog 另有 17 个经许可引入、但不在原始 400 名称中的 Skill；其中一个
是本项目为大目录渐进加载新增的 `all-skills-router`。因此 `full` 实际随包安装
320 个：319 个工作 Skill 加一个总目录路由 Skill。`lark-official` 是独立的
27 个官方来源 Skill 锁定集；在它的当前版本仍与官方端点一致时，合计可安装
347 个。上游更新不会改变或损坏内置 320 个，只会把外部阶段标记为
`update_available`，等待新锁审核。
账本公布非私有名称、匿名私有条目、去向、理由和替代项，不公布私有项目原名、
本机路径、内容哈希或私有审计信息。
随包分发的 320 个 Skill 另有统一机器目录
...
变化就把它报告为可用更新并停止外部写入，等待维护者重新审核。内置阶段仍然
健康。两段各自确认、各自记录安装状态，也能分别检查、更新判断、卸载和恢复。
...
外部官方 Skill 使用三层版本身份，不与 Kit 本体 hash 混用：

- Catalog 的 `sha256`：当前 Kit 已审核并允许新安装的版本；
- `previous_sha256`：曾审核过的旧版本，使已有安装在锁更新后仍可诊断和恢复；
- 项目内 `external-state.json` 的 `installed_sha256`：该项目实际安装的版本。
...
退出码 `0` 表示仍与锁一致，`1` 表示发现更新，`2` 表示网络、来源、名称或
回滚异常。发现更新不会修改 Catalog 或目标项目，也不会让内置包验证失败。
维护者审核官方页面后可运行
`python3 scripts/refresh_external_lock.py --apply --report /tmp/external-lock.md`；
它把旧 hash 移入历史并生成审查清单。GitHub 每日只读获取同一批官方端点；
发现变化时只更新专用候选分支并创建或刷新 PR，不自动安装或合并。PR 必须人工
查看每个官方 URL 后才能合并。仓库策略若禁止 GitHub Actions 创建 PR，定时
任务会停在该权限边界，维护者仍可使用上面的本地命令生成候选锁和审查清单。
...
python3 scripts/release_preflight.py \
  --source cursor-vault=/path/to/current-project/.cursor/skills \
  --source agents-global=/path/to/.agents/skills \
  --source codex-global=/path/to/.codex/skills \
  --private-inventory .agent-config-kit-workbench/private-skill-inventory.json
```

只有命令输出 `RELEASE PREFLIGHT OK` 才允许发布；`review`、`catalog_only`
或 `unexplained` 任一非零都会停止。`exact` 表示完整 Skill 目录一致；
`portable_derivative` 表示每个内容差异都有匹配的来源路径、来源锚点、源哈希、
包哈希和确定性便携化证明；公开目录只保留内容哈希，不公开维护者的源文件路径
...
正确；`scripts/verify.py` 只供 CI 和包内诊断，其成功信息 `PACKAGE VERIFY OK`
不依赖维护者私有账本。语义隐私检查只由 `release_preflight.py` 在发布前调用；
公开克隆无需、也无法获得私人词账本。
不能授权发布，也不等于维护者
真源已经对齐，也不等于商业 Runtime 的真实账号会话已经加载。真实测试必须在全新项目、
